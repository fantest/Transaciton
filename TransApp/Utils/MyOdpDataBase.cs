...
namespace FanTest.Trans
{
    ...
    /// Oracle数据访问对象的基类
    /// </summary>
    ///  Copyright (c) 2008-2010 中联信息产业公司
    ///  创 建 人：刘乐光
    ///  创建日期：2009-05-14
    ///  修 改 人：
    ///  修改内容：
    ///  修改日期：
    public class MyOdpDataBase : DataAccessBase
    {
        int blobSize = -1;
        ...
        {
            if (MyTransactionScope.Current != null)
            {
                DbConnection conn = MyTransactionScope.Current.GetConnection(DBConnString);
                if (conn == null)
                {
                    conn = new OracleConnection(DBConnString);
                    MyTransactionScope.Current.JoinConnectionToTransaction(DBConnString, conn);
                }
        ...
        OracleTransaction GetTransaction()
        {
            if (MyTransactionScope.Current != null)
            {
                DbTransaction trans = MyTransactionScope.Current.GetTransaction(DBConnString);
                if (trans == null)
                {
        ...
        public OracleDataReader ExecuteReader(string query, OracleParameter[] parameters, CommandType type)
        {
            ...
            {
                if (conn != null && MyTransactionScope.Current == null)
                {
                    conn.Clone();
                    conn.Dispose();
                }
